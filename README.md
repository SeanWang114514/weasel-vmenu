...
* **编码打完 + 回车 = 直接调用**：输入与某条编码**完全一致**时，回车把内容上屏
  （字母编码、纯数字编码都支持）。
* **候选窗口默认收起 9 个、按一次 `↓` 展开成 4 行 × 9 列，再按 `↓` / `↑` 收回**（见 §4
  「候选框：收起 / 展开」；**方向键不能移动选中项，选词用数字键**，原因见 §4）。
...
候选框**默认收起**（9 个候选，因为带序号注释，现在会排成 **2 行**），
**按一次 `↓` 展开成 4 行 × 9 列**的方格；**已展开时再按 `↓`（或按 `↑`）就收回**。

| 按键 | 行为 |
| --- | --- |
| `↓`（收起时） | **展开**成 4 行 × 9 列（当前选中项不变）✅ |
| `↓`（已展开） | ✅ **收回**（窗口高 287 → 144）——0.2.3 这里会误上屏，0.2.4 已修 |
| `↑`（展开后） | ✅ **收回** |
| `←` / `→` | ✗ **不会移动选中项**（原生导航器在这条链里不生效，见下面的说明） |
| 继续打字 / 上屏 / 数字选词 | 自动收回 |

> ⚠️ **已知限制：方向键不能移动选中项**（不是没写，是**没有 API**）。
> 实测对照：数字键正常（`1` → 是、`2` → 师），但 `→` 按 1 次或 3 次再按空格，
> 上屏的仍是第 1 个「是」；展开后 `↓↓` + `→` 也一样。
> 本机 librime-lua 里 `ctx.select_candidate` / `set_selected_candidate_index` / `highlight` /
> `move_selection` / `get_menu` / `selected_candidate_index` **全都不存在（`nil`）**，
> 而 `ctx.select` 是**「选中并上屏」**——用它「跳行」就会把候选打出去（0.2.3 踩过这个坑，见 `[0.2.4]`）。
> **选词请用数字键**（`1`–`9`，第 10 个是 `0`）或空格。想支持方向键移动，只能改小狼毫 C++ 源码。

底层是三个主题 / 方案配置键（都不在 Lua 里，也**不能运行时切换**）：
...
| `style/label_format` | Weasel 主题（同 `weasel.custom.yaml`） | 候选**原生序号列**的格式；原值是 `"%s"`，留空就**不再画序号** | `" "`（留空） |
...
**序号（第一行 1–9、下面几行干净）是这么来的** —— 三段配合：

1. 主题里把 `style/label_format` 留空 → Weasel **不再画原生序号**（连第 10 个之后的 `10`、`11` 一起消失）；
2. `menu_filter.lua` 的 `number_row1(cand, i)` 给**前 9 个候选**的 **comment** 写 `1`–`9`；
   注释只用于显示，**不进上屏文字**（实测 `shi`+空格 → 是、`shi`+`3` → 师）；
3. 同一个文件里 **v 菜单也把序号写进注释**（注释本来就带数字的，如快捷输入的「按 1 · …」，不重复加），
   否则关掉原生序号后 v 菜单就看不出按几（实测 `v`→`5`→`2` → 2026-09-13、`v`→`3` → 常用语）。

> 序号是**按「候选下标 1–9」**写的，不是「按行」：所以**收起状态**下这 9 个候选被序号撑宽、
> 排成 **2 行**时，第 2 行那两个（`8`、`9`）**仍然带数字**。
> 需求里的「下面几行不用序号」指的是**展开后的 4 行**（第 2–4 行 = 第 10–36 个候选，
> 没有注释 → 干净）✅ —— 这一点已经读图确认。

> 单纯靠 rime 的 `menu/alternative_select_labels` **做不到**：实测它对 Weasel 的渲染没用 ——
> 写进 `rime_ice.custom.yaml` 的 patch（36 项 / 10 项两种长度）会被整份拒绝（`page_size` 掉回 9、
> lua 挂载点消失）；直接写进 `build/rime_ice.schema.yaml`（前 9 项为 `1`–`9`、后面用空串或零宽空格）
> YAML 能过、窗口也正常，但**序号列的像素分布逐段完全相同**。所以序号只能走 `label_format` + 注释。
> （`build/rime_ice.schema.yaml` 里那行 36 项的 `alternative_select_labels` 目前还留着，对显示无影响。）

实测（候选窗口高度，物理像素；数字来自第六轮复测，`shots\` 里`show-1/2-*.png` 是同一状态的取证截图，高 145 / 285）：

| 操作 | 候选窗口高 | 结果 |
| --- | --- | --- |
| 打 `shi` | **144**（截图 145） | 收起：9 个候选排成 **2 行**，带序号 **1–7**（第 1 行）+ **8、9**（第 2 行） |
| 按 1 次 `↓` | **287**（截图 285） | **4 行 × 9 列**；只有第一行有 1–9，第 2–4 行直接从候选文字开始（读图确认）✅ |
| 再按 1 次 `↓` | 回到 **144** | ✅ **收回**（0.2.3 在这里会误上屏；0.2.4 已修） |
| 再按 `↓` 又按 `↑` | 287 → 144 | ✅ 展开 / 收回往复正常，**全程标题是 `*shi`、没有候选被上屏** |
| 再打任何字 | — | 自动收回 |

> 旧数据说明：**加序号之前**收起状态实测是 **797 × 74**（正好一行 9 个、宽度顶到 `530`）。
> 加了 `1`–`9` 注释后每个候选变宽，9 个在 530 px 里放不下 → 现在收起也是 2 行（高 144/145）。
> 另外「`↓↓` 与 `↓` + `→`×9 选中同一个候选」这条旧结论**已复测为误判**：当时第二次 `↓` 会直接上屏，
> 「比较」的其实是被意外上屏的那个字。

**回退方法**：`max_width` 改回 `0`、`page_size` 改回 `9`、`label_format` 改回 `"%s"`，然后重启 `WeaselServer`
（就是回到「一条长候选栏 + 原生序号」的原版行为）。
...
> 小瑕疵：v 菜单的序号是**逐条**写的（不受「只写前 9 条」限制），而快捷输入子菜单有
> 10 条（9 项 + 返回），所以「返回」那条前面会显示 `10`（它的实际按键是 `q`）。只影响观感。
...
* **候选框的「单行 / 4 行 × 9 列」是要改配置的**：宽度上限由 Weasel 主题的
  `style/layout/max_width`（现在 `530`）、一页条数由方案的 `menu/page_size`（现在 `36`）、
  原生序号列由主题的 `style/label_format`（现在留空）决定，改完必须重启 `WeaselServer`，
  **输入法里没有运行时开关**（见 §4「候选框：收起 / 展开」的开关方法与回退方法）。
* **序号是「展开后第一行 1–9、第 2–4 行没有」**：靠主题 `style/label_format` 留空 +
  `menu_filter.lua` 的 `number_row1` 把序号写进候选注释实现（注释不进上屏文字）。
  序号按**候选下标**写，所以**收起状态下**那 9 个候选排成两行时，第 2 行的 `8`、`9` 仍带数字。
  rime 的 `menu/alternative_select_labels` 对 Weasel **无效**，别拿它去改序号（见 §4）。
* **已知限制：方向键不能移动选中项**（`↓` / `↑` 只负责展开 / 收回）。
  本机 librime-lua 没有「移动高亮」的 API，`ctx.select` 又是「选中并上屏」——
  用它跳行会把候选打出去（0.2.3 踩过、0.2.4 已改掉）。**选词请用数字键**（见 §4）。
