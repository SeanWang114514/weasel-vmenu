...
它做的事：开记事本 → 打字 → 分别截图 → 对**候选区**做行/列投影与逐像素差分 →
打印每张图的「深底横带」行数和结论。
...
| 断言 | 判定方法 | 实测结果 |
|---|---|---|
| 收起态**只有一行** | 候选区深底横带 = 1 段、高约 64px | ✅ `zhe`：1 段，y 196–256 |
| 按 `↓` 展开 4 行 × 9 列 | 深底横带变成 1 大段、高约 204px（`zhe` 只有 3 行候选） | ✅ y 196–396 |
| `↓↓` **不收起** | 第 2 次 `↓` 后高亮移动、窗口高度不变 | ✅ 差分 12767 点，x[90..212] y[197..327] |
| `→` 在同一行内右移 | 差分集中在行内、y 范围不变 | ✅ 11374 点，x[90..294] |
| 第 1 行按 `↑` **收起** | 高度回到 64px | ✅ 1 段，y 196–256 |
| 序号在**词前** | 视觉读出 `1 这个 2 这股 …` | ✅（标签槽补丁后） |
| **按高亮行重新编号** | `↓↓` 后视觉读出第 2 行是 `1 这关 2 遮光`，第 1 行无序号 | ✅ |
| `+` 下翻 | 三页候选不重复、第 4 次回到第 0 页 | ✅ 见 §3.1 |
