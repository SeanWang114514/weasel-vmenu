...
| 文字块 / 注释块位置 | `ink.ps1 -In <crop> -Y0 108 -Y1 154 -Cells @(20,540,1060)` | 每格「文字块 → 右对齐注释块」；**剪贴板的注释已按用户要求删掉**（见下一行） |
| 注释有没有真的去掉 | 同上，看格内**墨迹连续段**有几块 | 剪贴板格 1 从「文字 `82..351` + 注释 `385..524`」变成**只有一块 `83..522`**（一直顶到格右缘）✅；常用语仍有第二块（触发键注释，属预期） |
