...
foreach ($line in $res.Lines) {
  $x = 99999; $y = 99999; $r = 0; $b = 0
  foreach ($w in $line.Words) {
    $rect = $w.BoundingRect
    if ($rect.X -lt $x) { $x = [int]$rect.X }
    if ($rect.Y -lt $y) { $y = [int]$rect.Y }
    if (($rect.X + $rect.Width) -gt $r) { $r = [int]($rect.X + $rect.Width) }
    if (($rect.Y + $rect.Height) -gt $b) { $b = [int]($rect.Y + $rect.Height) }
  }
  [void]$sb.AppendLine("[x=$x y=$y w=$($r-$x) h=$($b-$y)] $($line.Text)")
}
