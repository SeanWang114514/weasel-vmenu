...
* **展开栏翻到第 2 页后收起、再展开，会停留在收起前那一页**（用户报障原话：
  「翻开到第二页 …按上键…关闭，然后再次打开…会默认打开之前关闭的时候显示的展开栏，
  关闭后重新打开无论如何显示默认的第一页」）。
  * **根因**：页码存在会话级 option `vmenu_page_0..15` 里；`grid_key` 的收起分支
    （第一行按 `↑`）只写 `vmenu_grid=false`、**不清页码**，重新展开时
    `menu_filter.window_of` 仍按 `old page × lim` 取窗口起点 → 恢复到关闭前那一页
    （同理收起后的单行也会停在第 2 屏的第 10–18 个候选）。
  * **修法**（`vmenu_core.lua` `grid_key`，纯 Lua、无需重编 Weasel）：
    **展开的瞬间**（`↓` 从收起态展开）与 **收起的瞬间**（`↑` 第一行收起）都先
    `page_reset` 再写 `grid`——中间帧始终是「第 1 屏」的正常画面；`page_set` 自带
    「值没变不写 option」守卫（页码本来就是 0 时只多一次 get_option、零额外重翻译），
    `pcall` 隔离保证展开/收起动作本身必完成；两处各加一条 `debug_log` 记录复位前页码。
  * **验证**：新脚本 `tools/grid-page-verify.ps1`（真机模拟按键）——A 基线 `shi`→`1` 得 `是`；
    B 展开(663x190)→`=` 翻第 2 屏→`1` 得 `式/室`（证明翻页生效）；C 第 2 屏→`↑` 收起(592x49)
    →`↓` 重展开(663x190)→`1` 得 `是`（== 基线，**bug 已修**）；D 收起态直接按 `1` 也得 `是`；
    E 回归 `shi`+空格正常上屏；F 日志见 `收起网格：复位前 page=1` → `展开网格：复位前 page=0`。
    回归探针 `v2-space-verify.ps1` 同样全过（vclip 空格/数字/退格不受影响）。
...
用户原话（紧接着 0.2.18 的三条又追加了两条）：

> 1. 在不开启折叠栏的时候候选词框的大小跟随这个候选词
>    打开后就同步成最长的候选词
> 2. 同时限制候选词整条栏的最长长度 如果超出就删除部分候选词
> 3. 把第一个 131 删除 只留下第二个（这个也最好不要显示序号）

#### ① 收起态自然宽度；展开态统一列宽

* **收起态（单行）不再强制等宽**：每格按候选词实际内容的自然宽度排，框总宽 = 各格宽度之和
  （格间留固定间距，末尾不留）；这样框紧贴内容、没有多余的空白。
  之前收起态也用统一列宽（= 最长候选宽度），比如 `shi` 9 格每格 71px → 663 宽，
  其中短候选（如 `十`）右边有一段空白。现在 `shi` 收起 **592×49**（自然宽度总和），
  比展开态的 663×190 窄了约 10%，视觉上更紧凑。
* **展开态（4 行网格）统一列宽 = 最长候选**：这就是「打开后就同步成最长的候选词」，
  各列严格对齐，上下行同一列像素级对齐。
* **实现**：`grid_uniform` 条件改为 `!static_menu && grid_multi_row`（只在多行即展开态启用），
  收起态走自然累加（`w += text_natural_w`）而非钉列号。
* **末格拉到右缘**的美化（原来的 `if (!grid_multi_row && !fixed_cells)`）也跟着收窄到
  `if (grid_uniform && !grid_multi_row && !fixed_cells)` —— 自然宽度下这行等于永远不触发，
  所以收起态最后一个候选的高亮块只到候选词自己的右缘，不再拉到面板右缘。

#### ② 宽度上限超了就删候选词（至少留 3 条，删不够才压窄）

* 收起态：按各格自然宽度求和，超出上限就**从尾部删候选**（删一条、再求和、直到 ≤ 上限），
  下限 `floor_cols = min(3, 候选数)`。实测 `gith`：自然总宽 ≈ 1300，上限 1100 →
  删 3 条剩 6 条 → 994×49 ✓ **没有 `…` 截断**（这是和 0.2.18 最大的区别）。
* 展开态：统一列宽 × 列数超标就**少放几列**（至多 3 列），少到 3 列还超标才压窄 + `…` 兜底。
* 两种状态的兜底都是：格宽压到 `row_budget / draw_cols`（下限 `120 + spacing`），
  文字/注释由 DirectWrite 裁成 `…`（候选文本本身不动）。

#### ③ 数字编码候选只留常用语预览，不画序号

* **Lua**：删掉 `menu_filter.lua` 的 `digit_cand` 函数和 `raw_digit` 分支。
  之前纯数字输入时候选有两条：`1 131 / 2 13122500717 常用语`。
  现在只剩**一条**：`13122500717 常用语`（无序号）。
* **服务端**：`RimeWithWeasel.cpp` 新增 `_VMenuSeqMode`（替代旧 `_VMenuSeqLabels`），
  返回 int 0/1/2；当候选数 = 1 且输入全为数字时返回 2 = 不画序号。
* **客户端**：`HorizontalLayout::GetLabelText` 对 `grid_2d == 2` 返回空字符串，
  标签槽宽度 = 0，候选词从左边开始（不加标签与文字之间的间距）。

#### 实测数据（逻辑坐标；屏宽 1707）

| 用例 | 0.2.19 | 0.2.18 |
| --- | --- | --- |
| `shi` 收起 | **592×49**（自然宽度） | 663×49（统一列宽） |
| `shi` 展开 | 663×190（统一列宽，未变） | 663×190 |
| `gith` 收起 | **994×49**（6 条自然宽度，删 3 条） | 1122×49（6 条统一 183 + `…`） |
| `131` 候选 | **193×49**（1 条，无序号） | 452×49（2 条，有 `1`/`2` 序号） |
| `v` 主菜单 | 849×49（未变） | 849×49 |
| `v`→`3` | 997×49（未变） | 997×49 |
| `13`+`-` | `13-` ✅ | `13-` ✅ |
| `131`+回车 | `13122500717` ✅ | `13122500717` ✅ |
| `wsl`+回车 | `wslzhenshuai@163.com` ✅ | `wslzhenshuai@163.com` ✅ |
| `ni`+`1` | `你` ✅ | `你` ✅ |



### 三条用户反馈：v 菜单排成一行 / 长候选框不再顶满屏 / 数字不再「变成候选态」

用户原话（一次性提了三条）：

> 1. 让 v 功能的页面的候选词在同一行显示
> 2. 在输入 GitHub 等词的时候会出现候选词框过长的问题 并且不止一个有这个问题
>    删除 1-3 个候选词不是让你拉长候选词框的
> 3. 输入数字的时候会变成候选状态 也就是无法在输入如 13 的时候直接按 "-" 等按键在后面插入一个 "-"，
>    但是要保留数字作为常用语的编码（如常用语 13122500717 在输入 131 的时候会在候选词中显示，
>    但是有且只有在输入回车后才会输入完整的常用语；输入 1 等词的时候不会输入而是正常地加在后面）

#### ① 静态 v 菜单排成一行（`HorizontalLayout.cpp` + 服务端 `_VMenuCols`）

* 静态菜单（`ctx.grid_2d == 0`）新增 `static_menu` 分支：**列数 = 候选条数**、强制单行、
  格宽用内容**自然宽度**（不再等宽居中），也不参与「丢格 / 压宽」。
* 服务端 `_VMenuCols` 对 `vqi`（快捷输入）**不再返回 2** —— 0.2.17 把它接进剪贴板那套
  2 列固定格，这轮用户要的是「一行」，所以退回静态菜单排版。
* **实测**：`v` 主菜单 `849x49`（5 项一行，原来 4 列 `861x95` 两行）、
  `v`→`3` 快捷输入 `997x49`（7 项一行，原来 `707x189` 四行）。
  剪贴板 `v`→`2` 仍是 `706x143`、常用语 `v`→`4` 仍是 `706x49`（用户没要求改，保持 2×3 固定格）。

#### ② 候选框宽度上限真的生效了（长英文候选不再拉满整屏）

* 之前「少放 1–3 列」的逻辑只看**屏宽**（`grid_budget`），而 `gith` 有 **9** 条候选
  （9 × 272px = 2448），`min_cols = 9 − 3 = 6` 先卡住了丢格循环，于是框还是 `1656x49`。
* 现在 `row_budget = min(style.max_width, grid_budget)`：丢到 3 条仍超就把**格宽压到**
  `row_budget / draw_cols`（下限 `120 + candidate_spacing`），多出来的字画 `…`
  —— **只裁绘制，候选文本不动，上屏仍是完整的**。
* 实测 `gith`：**`1656x49` → `1122x49`**（逻辑屏宽 1707），一行 6 个：
  `1 GitHub · 2 GitHub Pages · 3 GitHub Actions · 4 GitHub Copilot · 5 GitHub Copilot… · 6 GitHub Codes…`。
* 想更宽/更窄改主题 `style/layout/max_width`（当前 `1100`），改完重启 `WeaselServer`。
* 已知代价（与旧特性同源）：被丢掉的第 7–9 条**仍能被数字键选中**（选词由服务端按下标算）。

#### ③ 数字输入不再「变成候选态」（`menu_processor.lua` + `menu_filter.lua`）

* 根因是两条老机制叠加：`menu_filter` 把「数字编码本身」做成候选，`punctuator` 见到有候选菜单时
  **先提交当前高亮的候选再插入符号**，于是 `13` + `-` 变成「13 上屏 + -」，看起来像被吃掉了。
* `menu_processor.lua` 新增分支（放在 raw 模式之后、翻页分支之前）：输入为空或全数字时 ——
  * 数字能继续延伸某条**纯数字编码** → 并进编码（`push_input`）；
  * 数字延伸不了 → **数字当普通字符上屏**（`1313` 就是 `1313`，`1312` 就是 `1312`）；
  * `-` / `=` / `+`（含小键盘）→ **先上屏数字本身，再插入符号** → `13` + `-` = `13-`；
  * `回车` → 仍然是**唯一**能上屏整条常用语的键。
* `menu_filter.lua` 新增 `digit_cand(code)`：全数字输入时「数字本身」**永远排第 1 位**（`quality` 最大），
  常用语排第 2 位 —— 这样输入框里不再是「只有一条常用语候选」的样子。
* 翻页键收窄：`-` / `=` 只在**列表型 v 功能**或**纯拼音输入**（`^[a-z']+$`）时才翻页。
* **实测**（干净记事本逐条）：`13`+`-`→`13-`；`131`+回车→`13122500717`；`1313`→`1313`；
  `13`+`.`→`13。`；`13`+`Shift+=`→`13+`；`13`+`=`→`13=`；`1312`→`1312`；`wsl`+回车→收藏邮箱；
  `ni`+`1`→`你`（中文打字无回归）。

#### 这一轮踩到的坑（留给下次）

* **测试脚本的 `Tap()` 必须传虚拟键码，不是字符码**：`'w'` 要发 `0x57`，
  写成 `[byte][char]'w'` 就成了 `0x6E` = 小键盘 `.`、`'i'`→`0x69` = 小键盘 `9`，
  于是「打字」变成了在小键盘上敲 `.9` —— 白白怀疑了输入法半个多小时。
* `vmenu_core.lua` 的 `DEBUG_CAND`（每个按键写一次候选表）默认关；排查「显示顺序对不上」时再开。
* `HorizontalLayout.cpp` 里为定位宽度上限临时加的 `VMENU_LAY_DEBUG`（写 `vmenu-layout.log`）**已删除**，
  正式构建里没有诊断代码。

## [0.2.17] - 2026-09-19

### 快捷输入（`v`→`3`）也改成 2 列固定格；顺手修好它的「返回」

* 用户诉求（原话）：「把快捷输入的候选词显示也改为和粘贴板相似的2*3格式」。
* **服务端一行改动**：`RimeWithWeasel.cpp` 的 `_VMenuCols` 对 `vqi`（快捷输入）也返回 **2**。
  客户端 `HorizontalLayout` 的 `fixed_cells` 只看 `ctx.grid_cols == 2`（与 `grid_2d` 无关），
  于是 v3 自动获得与剪贴板同一套排版：**等宽固定格 + 屏宽 20% + 超长 `…` 截断**。
* v3 仍是**静态菜单**：序号顺序 1..7（`ctx.grid_2d = 0`），数字键继续由 `menu_processor.lua`
  按序号选，服务端不接管 —— 序号画的和按下去选的完全一致。
* **实测**：`v`→`3` 面板 `707x189`（2 列；7 个条目排成 4 行），格宽与剪贴板一致（`707` 宽）；
  注释「按 c / 按 r / 按 s / 按 n / 按 h / 按 u / 按 q」保留并右对齐；
  `v` 主菜单仍是 4 列 `861x95`，剪贴板仍是 `707x144`（都没受影响）。
* **7 个条目 → 4 行**（2 列 × 3 行只放得下 6 个）：第 4 行只有第 1 格（返回）。
  要严格 2×3 的话，只能把「返回」挪到第二页或从菜单里去掉 —— 两条都会让某个条目不再一眼可见，
  所以先按「全部可见」实现，等用户拍板。
* **顺手修的 bug**：v3 的「返回」（按 `7` 或按 `q`）以前**没有反应** ——
  它走的是 `ctx:clear()`，在这条 processor 路径上不生效（输入框仍是 `vqi`，面板不消失）。
  改成 `replace_input(ctx, "v")` 后真的回到 v 主菜单（实测面板 `861x95`、预编辑「功能菜单」）。
* 记事本与网页（独立 profile Chrome）实测一致（`707x189`）。
